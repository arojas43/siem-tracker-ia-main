...
import classes from './OneDateCheckboxOnYesCommentCard.module.scss';
...
const OneDateCheckboxOnYesCommentCard: FC<TaskActionFormProps> = ({ onClose, taskId, taskActionRequirements }) => {
    ...
    // const [checkboxValue, setCheckboxValue] = useState<string>(taskActionRequirements?.checkboxValue || '');
    // const [checkboxValue, setCheckboxValue] = useState<string>(taskActionRequirements?.checkboxValue ? 'si' : 'no');
    ...
    useEffect(() => {
        if (
            ...
            (isDateSubmitable && checkboxValue === 'si' && commentValue)
        ) {
            setCanSubmit(true);
            return;
        }
        setCanSubmit(false);
    }, [isDateSubmitable, checkboxValue, commentValue]);
    ...
    const handleSubmitForm = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const taskActionData = {
            taskActionType: 'one_date_checkbox_onYes_comments',
            taskActionRequirements: {
                ...
                // checkboxValue: formDataHolder.get('checkboxValue') as string,
    ...
    const isFormInvalid = ({ firstFieldValue, commentValue }: TaskActionRequirements): boolean => {
        setFormInputsValidator({
            firstDateIsInvalid: !firstFieldValue && !!taskActionRequirements?.firstDateRequired,
            secondInputIsInvalid: !commentValue && checkboxValue === 'si',
        });
        ...
        <>
            ...
            >
                    ...
                    <Col>
                        <Form.Control
                            ...
                            onChange={({ currentTarget: { value } }) => {
                                ...
                                // handleInputChanged(value, 'firstDateIsInvalid');
                            }}
                ...
                <Form.Group as={Row}>
                    ...
                    <Col>
                        <Form.Control
                            disabled={!checkboxValue || checkboxValue === 'no'}
                            as="textarea"
                            name="commentValue"
                            value={commentValue}
                            onChange={({ currentTarget: { value } }) => {
...
export default OneDateCheckboxOnYesCommentCard;
